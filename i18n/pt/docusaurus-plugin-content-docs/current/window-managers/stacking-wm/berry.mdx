...
**Rofi** é usado para iniciadores de aplicativos, executores de comandos e vários applets. Para modificar a configuração do rofi, edite o arquivo `~/.config/rofi/config.rasi`.

> Se precisar de ajuda com a configuração do rofi, execute: `man rofi` no terminal.

Os scripts para o rofi usados no Berry podem ser encontrados no diretório `~/.config/berry/scripts`. Estes são os seguintes scripts que utilizam o rofi para fornecer os iniciadores e applets no Berry:
...
**`rofi_launcher`** : Iniciador de aplicativos, executor de comandos, gerenciador de arquivos e gerenciador de tarefas  <br/>
...
Suponha que você queira modificar a aparência do **iniciador**, então você precisa editar o arquivo `~/.config/berry/theme/rofi/launcher.rasi`. O mesmo vale para qualquer outro applet, edite o arquivo respectivo no mesmo diretório.

> Se precisar de ajuda com a personalização do rofi, execute: `man rofi-theme` no terminal.

Para mudar as cores e fontes dos iniciadores e applets, edite os arquivos `~/.config/berry/theme/rofi/shared/colors.rasi` e `~/.config/berry/theme/rofi/shared/fonts.rasi`.
