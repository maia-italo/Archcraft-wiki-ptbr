...
Os scripts para rofi usados no Blackbox podem ser encontrados no diretório `~/.blackbox/scripts`. Estes são os seguintes scripts que usam o rofi para fornecer os iniciadores e applets no Blackbox:
...
Para mudar as cores e fontes dos iniciadores e applets, edite os arquivos `~/.blackbox/theme/rofi/shared/colors.rasi` e `~/.blackbox/theme/rofi/shared/fonts.rasi`.
